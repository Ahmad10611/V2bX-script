...
install() {
    bash <(curl -Ls https://raw.githubusercontents.com/Ahmad10611/V2bX-script/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/Ahmad10611/V2bX-script/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/V2bX -N --no-check-certificate https://raw.githubusercontent.com/Ahmad10611/V2bX-script/master/V2bX.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/Ahmad10611/V2bX ---
  ${green}0.${plain} 修改配置
————————————————
